# Embedded file name: /usr/lib/enigma2/python/Plugins/Extensions/TuneinRadio/addons/radio/TuneinRadio/tunein.py
...
def get_max_preset_num(elementslist):
    ...
    for element in elementslist:
        if 'station' in element and get_int(element['station'], 'preset_number') > maxpresetnum:
...
def process_tunein_json(elements):
            ...
            if 'children' in element:
                for children in element['children']:
                    if 'key' in element and element['key'] == 'shows':
                        ...
                        pass
                    elif 'type' in children and children['type'] == 'audio':
                        if 'item' in children and children['item'] == 'station':
                            elementslist.append({'station': __read_element__(children)})
                            ...
                            if presets:
                                elementslist.insert(children['preset_number'], {'show': __read_element__(children)})
                            else:
                                elementslist.append({'show': __read_element__(children)})
                    elif 'type' in children and children['type'] == 'link':
                        elementslist.append({'link': __read_element__(children)})
    ...
    for element in elementslist:
        if 'link' in element:
            ...
            id = items['preset_id']
            try:
                extra = items['subtext'].encode('utf-8')
            except:
                extra = ''
...
class TuneIn:
    ...
    def __init__(self, partnerid, serial = None, locale = 'en-GB', formats = None, https = True, debug = False):
        ...
        self._global_params = []
        self._global_params.append({'param': 'partnerId',
         'value': partnerid})
        if serial is not None:
            self._global_params.append({'param': 'serial',
             'value': serial})
        self._global_params.append({'param': 'render',
         'value': 'json'})
        self._global_params.append({'param': 'locale',
         'value': locale})
        if formats is not None:
            self._global_params.append({'param': 'formats',
             'value': formats})
        self._debug = debug
        ...
        for node in asx.childNodes:
            if str(node.localName).lower() == 'entryref' and node.hasAttribute('href'):
                ...
                for subnode in node.childNodes:
                    if str(subnode.localName).lower() == 'ref' and subnode.hasAttribute('href') and subnode.getAttribute('href') not in streams:
                        streams.append(subnode.getAttribute('href'))
                    elif str(subnode.localName).lower() == 'ref' and subnode.hasAttribute('HREF') and subnode.getAttribute('HREF') not in streams:
                        streams.append(subnode.getAttribute('HREF'))
    ...
    def account_reset(self, username = None, email = None):
        ...
        """
        params = [{'param': 'c',
          'value': 'reset'}, {'param': 'username',
          'value': username}, {'param': 'email',
          'value': email}]
        result = self.__call_tunein('Account.ashx', params)
        if self.__result_ok(result):
            raise TuneIn.TuneInError(self.__result_status(result), self.__result_fault(result), self.__result_fault_code(result))
    ...
    def browse_presets(self, username = None, formats = None):
        ...
        """
        params = [{'param': 'c',
          'value': 'presets'}, {'param': 'username',
          'value': username}, {'param': 'formats',
          'value': formats}]
        result = self.__call_tunein('Browse.ashx', params)
        if self.__result_ok(result):
            raise TuneIn.TuneInError(self.__result_status(result), self.__result_fault(result), self.__result_fault_code(result))
    ...
    def preset_listfolders(self, username = None, password = None):
        ...
        """
        params = [{'param': 'c',
          'value': 'listFolders'}, {'param': 'username',
          'value': username}, {'param': 'password',
          'value': password}]
        result = self.__call_tunein('Preset.ashx', params)
        if self.__result_ok(result):
